...
import { loadKeys, getKey } from "@/lib/storage";
...
export default function Home() {
  ...
  // Detect missing keys on mount and settings close
  useEffect(() => {
    const keys = loadKeys();
    const missing: AIModel[] = (["claude", "openai", "gemini"] as AIModel[]).filter((m) => !keys[m]);
  ...
  const handleGenerate = async () => {
    ...
    if (!apiKey) {
      setError(`No API key for ${model}. Click the ⚙ Settings icon to add it.`);
    ...
    try {
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({
          images: images.slice(0, 3), // send up to 3 images (vision token cost)
              ...
              <div className="space-y-2">
                {[
                  ...
                  >
                    ...
                    <div className={`h-4.5 relative h-5 w-9 rounded-full transition-colors ${value ? "bg-brand-600" : "bg-[#2e2e3a]"}`}>
